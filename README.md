...
- **ZEAL voice command:** in command mode, prefix with "ZEAL …" (or "note …") and the instruction is sent to your ZEAL agent's `/api/voice` endpoint — ZEAL executes it (tasks, missions, kanban, etc.) and replies; the reply pops as a notification and, with **Speak ZEAL replies** on, is read aloud via the sidecar's edge-tts. Configure the endpoint + key in Settings → ZEAL.
